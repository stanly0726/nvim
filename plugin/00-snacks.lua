...
vim.schedule(function()
  bind.load_mapping({
    ...
    ['n|<leader>sw'] = bind
      ...
      :with_desc('Snacks: workspace grep'),
    ['n|<leader>sW'] = bind
      .map_lua(function()
        Snacks.picker.grep_word({ hidden = true })
      end)
      :with_desc('Snacks: grep under cursor'),
    ['n|<leader>ss'] = bind
      .map_lua(function()
        Snacks.picker()
      end)
      :with_desc('Snacks: picker menu'),
    ['n|<leader>sd'] = bind
      .map_lua(Snacks.picker.diagnostics)
      :with_desc('Snacks: diagnostics picker'),
    ['n|<leader>e'] = bind
      .map_lua(function()
        Snacks.explorer({ enter = false })
      end)
      :with_desc('Snacks: explorer'),
  })
